...
writer = SummaryWriter(f'/home/xiao/code/CS5242/CS5242/tfboard/drop_flip5')
...
if __name__=="__main__":
    net = LeNet(2,config)
    trainset = ImageDataset(f'/home/xiao/code/CS5242/dataset_copy/train/',device=device,config=config,train=True)
    trainloader = DataLoader(trainset, batch_size=config["batch"], shuffle=True, num_workers=16,drop_last=True)
    testset = ImageDataset(f'/home/xiao/code/CS5242/dataset_copy/test/',device=device,config=config,train=True)

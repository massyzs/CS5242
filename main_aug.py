...
parser.add_argument('--rate', type=float, default=0.5, help="bool: whether or not to use data augmentation")
...
writer = SummaryWriter(f'/home/xiao/code/CS5242/CS5242/tfboard/{args.norm_type}{args.norm}_{args.rate}_aug_mix{args.ratio}')
...
if __name__=="__main__":
    ...
    trainset = ImageDataset(f'/home/xiao/code/CS5242/dataset_aug/{args.ratio}_in_one/train/',device=device,config=config,train=True)
    trainloader = DataLoader(trainset, batch_size=config["batch"], shuffle=True, num_workers=16,drop_last=True)
    testset = ImageDataset(f'/home/xiao/code/CS5242/dataset_aug/{args.ratio}_in_one/test/',device=device,config=config,train=True)
